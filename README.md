...
- **RegExp:** Thompson NFA + PikeVM engine, `/pattern/flags` literals, `RegExp()` constructor (new + plain call, RegExp/string patterns, flag validation with SyntaxError), `exec`/`test` with RegExpBuiltinExec semantics (global/sticky `lastIndex` advance, sticky failure reset), match results with `.index`/`.input`, `.source`/`.flags`/`.lastIndex` properties, lookahead `(?=…)`/`(?!…)`, `{n,m}` quantifiers, **anchors `^`/`$` (start/end, via `Edge::AnchorStart/AnchorEnd` + pos==0/pos==len checks, handles `^$` empty string and `$` zero-length at end, sticky-aware)**, replace with `$&`/``$` ``/`$'`/`$1..$n` expansion and function replacement (incl. `replaceAll` function replacement)
