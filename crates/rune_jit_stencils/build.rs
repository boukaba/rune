...
impl Emitter {
    ...
    fn add_helper(&mut self, h: &Helper) {
    ...
    fn add_stencil(&mut self, s: Stencil) {
            ...
            if s.holes.is_empty() {
                ...
                out.push_str(&format!(
                    ...
                    s.holes
                        ...
                        .map(|h| {
                            format!(
                                "HoleDef {{ byte_offset: {}, bit_offset: {}, bit_width: {} }}",
            ...
            if s.link_holes.is_empty() {
                ...
                out.push_str(&format!(
                    ...
                    s.link_holes
                        ...
                        .map(|lh| {
                            format!(
                                "LinkHoleDef {{ byte_offset: {}, helper_name: \"{}\" }}",
...
fn extract_text_section(o_bytes: &[u8]) -> Option<&[u8]> {
    ...
    if u32::from_le_bytes(o_bytes[0..4].try_into().unwrap()) != MH_MAGIC_64 {
        return None;
    }
...
fn compile_helper(stencil_dir: &Path, name: &str) -> Helper {
    ...
    let section =
        extract_text_section(&obj).unwrap_or_else(|| panic!("helper {name}: no text section"));
    ...
    assert!(
        section.len() >= 16,
        "helper {name}: section too small: {} bytes",
        section.len()
    );
    ...
    assert_eq!(
        ...
        "helper {name}: expected prologue {:#010x}, got {:#010x}",
        expected_prologue, prologue
    );
    ...
    assert_eq!(
        ...
        "helper {name}: expected RET at end ({:#010x}), got {:#010x}",
        expected_ret, ret_actual
    );

    let epilogue_offset = ret_offset - 4;
    let epilogue = u32::from_le_bytes(
        section[epilogue_offset..epilogue_offset + 4]
            .try_into()
            .unwrap(),
    );
    assert_eq!(
    ...
    assert_eq!(
        body.len() % 4,
        0,
        "helper {name}: body length {} not multiple of 4",
        body.len()
    );
...
fn emit_naked_stencil(
    emitter: &mut Emitter,
    stencil_dir: &Path,
    name: &str,
    checks: &[(usize, u32, &str)],
    holes: &[HoleSpec],
    body_len: usize,
) {
    let obj = compile_c(stencil_dir, name);
    ...
    for (offset, expected, desc) in checks {
        ...
        assert_eq!(
            instr, *expected,
            "{name}[{}]: expected {} ({:#010x}), got {:#010x}",
            offset, desc, expected, instr
        );
    }
...
fn emit_real_c_stencil(
    emitter: &mut Emitter,
    stencil_dir: &Path,
    name: &str,
    value_checks: &[ValueCheck],
    holes: &[HoleSpec],
    link_holes: &[LinkHoleSpec],
) {
    let obj = compile_c(stencil_dir, name);
    let section = extract_text_section(&obj).unwrap_or_else(|| panic!("{name}: no text section"));
    ...
    let last_value_offset = value_checks
        .iter()
        .map(|vc| vc.offset + 4)
        .max()
        .unwrap_or(0);
    // The branch instruction is a link hole — include it in the body.
    let branch_ends_at = link_holes
        .iter()
        .map(|lh| lh.byte_offset + 4)
        .max()
        .unwrap_or(0);
