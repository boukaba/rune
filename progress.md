...
| **5l** | Remaining JIT opcodes (floats, property access, calls) | 2d | 🟠 P1 | 🟡 In progress | Next priority: Neg, Not (unary Smi ops, no GC), then LoadFloat64, LoadPropertyIC. |
...
- JIT: 24/61 opcodes covered (Smi arithmetic + comparison + bitwise + branches + locals). Skipped for tiny functions (<20 instrs). Both x86-64 and AArch64 backends functional.
...
**Gaps (documented):** No standard library, optimizing JIT (remaining 37/61 opcodes), modules, classes, async/await. 5–230× slower than V8 on hot loops. JIT covers 24/61 opcodes (Smi arithmetic + comparison + bitwise; missing floats, property access, calls).
