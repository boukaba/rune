...
- **Array methods:** `filter`, `map`, `reduce`, `forEach`, `slice` (callback state machine, GC-safe across 200K elements), `find`, `findIndex`, `some`, `every`, `sort`, `flat`, `flatMap`, `includes`, `push`, `pop`, `shift`, `unshift`, `splice`, `concat`, `reverse`, `indexOf`, `join`
...
- **Array methods:** concat spread pushes `undefined` for absent source indices (dense arrays cannot represent holes); no 2^53 length guards (u32 lengths).
