...
| `rune_jit_baseline` | Baseline JIT (AArch64 + x86-64) — 62 opcodes whitelisted, function tier-up at 50 calls, N=16 vector IC table, **inlining** (Phase F: hot callees spliced inline, ~5% gain), **float self-tagging** (NaN-boxed Values, 0 heap allocation for floats) |
...
- **Arrays:** dense arrays with real holes (elisions `[1, , 3]` parse; `delete` punches holes; holes fall through to the prototype, join as `""`, spread as `undefined`), `defineProperty` on indices (data + getter/setter overlay), spread, destructuring (declarations + assignment), rest, push/pop/length
...
- **RegExp:** Thompson NFA + PikeVM engine, `/pattern/flags` literals, `RegExp()` constructor (new + plain call, RegExp/string patterns, flag validation with SyntaxError), `exec`/`test` with RegExpBuiltinExec semantics (global/sticky `lastIndex` advance, sticky failure reset), match results with `.index`/`.input`, `.source`/`.flags`/`.lastIndex` properties, lookahead `(?=…)`/`(?!…)`, `{n,m}` quantifiers, **anchors `^`/`$` + word boundaries `\b`/`\B` + backrefs `\1..9` + flags `i/m/s` (case-fold, multiline `^`/`$` via `m`, dotAll via `s`)**, replace with `$&`/``$` ``/`$'`/`$1..$n` expansion and function replacement (incl. `replaceAll` function replacement)
- **Array methods:** `reduce`, `forEach`, `find`, `findIndex`, `some`, `every`, `sort` (stable merge, user comparators, observable Get/Set element access, holes trail), `toSorted` (copy), `flat`, `flatMap`, `includes`, `push`/`pop`/`shift`/`unshift`/`reverse`/`slice`/`splice`/`concat`/`map`/`filter` (spec-audited: generic receivers, holes, u64 lengths, 2^53 overflow throws, exact returns, IsConcatSpreadable, species validation + species-plain results, hole-preserving results, JS length dispatch via LengthOfArrayLike machine, species Get dispatch with JS getters, custom-ctor Construct + install, `Array.of`/`from` custom-this), `indexOf`, `join`, `copyWithin`, `toSpliced`, `with`, `of`, `reduceRight`, `findLast`, `findLastIndex`, `lastIndexOf`, `fill` (in-place, returns `this`), `toReversed` (descending copy, holes materialize as `undefined`, no species)
- **Math / Object / Function statics:** Math `round/trunc/sign/hypot/clz32/imul/cbrt/log/log2/log10/exp/sin/cos/tan/asin/acos/atan/atan2` + constants (`PI/E/LN2/LN10/LOG2E/LOG10E/SQRT1_2/SQRT2`) with spec-exact semantics (`round` ties toward +∞); Object `assign/is/getOwnPropertyNames/fromEntries/hasOwn/setPrototypeOf`; Function.prototype `apply` (array-like argArray)
...
- **Global functions:** `parseInt` (radix, hex), `parseFloat` (Infinity, NaN, scientific notation), `isNaN`, `isFinite` (ToNumber coercion, symbols throw)

## What Doesn't Work (Yet)

- **Standard library:** No WeakRef. Typed arrays: ctor accepts arrays/strings/typed arrays/ArrayBuffers but not general iterables; no BigInt64Array/BigUint64Array/Float16Array; non-numeric own props on typed arrays unsupported; no `Object.prototype.toString` builtin (so `@@toStringTag` is unobservable). Date is UTC-only (no local timezone — spec-conformant, `getTimezoneOffset` always 0) and `toLocaleString` family = `toString` family (no ECMA-402); no `setYear`/`getYear`/`toGMTString` (Annex B); `Date.prototype.toJSON` handles Date receivers only. Map/Set forEach is O(n²) worst case (linear live-presence check per snapshot key). RegExp: `v` flag accepted by the ctor but the literal parser's flag whitelist predates it (`/a/v` literal unsupported); match-result `index`/`input` enumerability is unobservable (no shape enum flags — `Object.keys` includes them); **flags `i/m/s` + anchors/backrefs fixed (C1-C4) — `exec_with_flags`;** `{n,m}` captures return the first copy's capture; engine unit is Rust char (no `u`-flag UTF-16 index math, leftmost still longest). Iteration: no IteratorClose on break/return, no `let`-per-iteration freshness in for..of, `.values()/.keys()/.entries()` need array receivers (no array-likes), destructuring LHS in for..of unsupported. Symbols: **`Number(sym)` and arithmetic now correctly throw `TypeError: Cannot convert a Symbol value to a number` (C5) — `ToNumber` via `handle_throw`/`pending_exception`, `+` splits string vs number correctly.**
...
- **Array methods:** holes exist (elisions/`delete`/`length` tails/`length`-shrink, huge indices as named overflow props) with hole-preserving `map` results, packed `filter`, hole-skipping `flatMap`, `new Array(n)` holes; `Array.from` (iterable drain incl. custom iterators with `IteratorClose`, array-like fallback with hole densification, mapfn, Array/Object ctor dispatch); array `length` is a real exotic (defineProperty/`length=` coercion with RangeError, shrink deletes with k+1 abort, non-writable lengths); `Object.freeze`/`seal`/`preventExtensions` work on arrays with `isFrozen`/`isSealed` checks; index descriptors via `getOwnPropertyDescriptor` (holes absent, overlays with attrs); `delete` honors configurability; lengths clamp at 2^53-1 but dense storage caps at 2^32-1.
...
- **JIT:** 62 opcodes whitelisted (out of ~93 opcode variants). Float Self-Tagging (NaN-boxing) eliminates all float heap allocation — all interpreter float paths use inline `Value::from_float64`. JIT has float64 Add promotion (full §12.8.5 string-concat semantics); Sub/Mul/Mod run natively as guarded Smi ops; **Sub/Mul/Div/Mod/Exp + Lt/Gt/Le/Ge all run native** with float fallback via `jit_binop_helper` (J1/J2); float literals eligible; float-promoted accumulators stay native with zero bailouts; optional chaining (`JumpIfNullOrUndefined`) runs native exact nullish branches; In/Instanceof bail to the interpreter. Compile-time `stack_depth` model + `push_raw` bail paths; every guard (shape-miss, Smi-check, overflow, call) bails to the interpreter with a validated pre-opcode snapshot. Phase F inlining shipped (5% on `jit_hot_function_1M`).
...
| **v0.11.1** ✅ | **Generator resumption + `yield*` completion**: `throw()` injects abrupt completion at the suspend point (in-generator catch/finally run; uncaught completes+propagates); `return()` runs finallys via a catch-invisible sentinel, yielding again when finally yields; `yield*` sent-value forwarding + delegate return value (`ForOfNextStar`/`YieldStarYield`); delegate throw/return forwarding incl. JS-method delegates, accessor getters, IteratorClose violation paths. 11 new integration tests (26/26 generator); GeneratorPrototype 34→47/61; 842/0 workspace. Known gap: GeneratorFunction ctor (needs runtime compilation — `Function` ctor likewise absent). |
| **v0.11.0** ✅ | **Real generator instances**: `function*` calls return JSObjects (next/return/throw/@@iterator, proper IterResults); executing-guard closes on violation/abrupt; try_stack + operand stack survive suspend/resume; for-of/spread/`yield*` work. 15 integration tests; GeneratorPrototype 6→34/61; 831/0 workspace. |
| **v0.10.0** ✅ | **let-for per-iteration semantics**: body/update write-splitting per §14.7.4.9 — body mutations survive iterations (the long-standing infinite-loop/SEGV family is dead: `for(let i=0;i<3;){i++;}` terminates), updates stay invisible to iteration closures, nested loops + labeled continues across let-for levels correct, block-shadow no longer clobbers loop vars. Continue suite 1→10. 820/0 workspace. |
| **v0.9.7** ✅ | **`for (x in obj)` bare/member LHS** parses (no-In grammar mode; previously `in` was eaten as a binary operator → parse error). for-in suite 9→14 (+5). 820/0 workspace. |
| **v0.9.6** ✅ | **Labeled statements** (`outer: for(...) { break outer; }`) — parser + label-aware loop frames + labeled break/continue resolution; **for-in break/continue machinery** added. statements/labeled 0→2, break 1→7, continue 1→3. 820/0 workspace. |
| **v0.9.5** ✅ | **JIT overflow/resume soundness**: root-caused the deep-recursion NaN (kept-frame zombies resumed against released JIT-stack windows) and replaced fine-grained resume with UNWIND-TO-OUTERMOST — each helper level re-records its own call-site + caller window so the interpreter re-executes the outermost Call interpreted with exact operands. Native-region budget raised 8→256 slots/frame (>8-deep native chains now run natively instead of bailing per level). fib(6..30) all exact incl. previously-corrupt depths; deep/mutual/float chain patterns verified. 820/0 workspace. |
| **v0.9.4** ✅ | **Top-level/sibling function & class declaration capture**: `function add(){}; function f(){ return add(1,2); }` works (was "undefined is not a function" — decls were invisible to inner functions). Script-level escape-analysis env captures ONLY declarations (vars stay global for cross-eval persistence); Function/Class statement bindings sync into the capture env; compile_function pre-scan includes decl names. **language/function-code 87→103 (+19%)**, 820/0 workspace. |
| **v0.9.3** ✅ | **JIT convention v2-lite + call-IC fix**: recursive hot functions no longer panic at tier-up — fib(6..24) exact. Universal callee Frames (call_helper + interpreter IC/tier-up sites) with call-site pc stamping and kept-on-pending frame chains; branch-target depth override map + binop slow-path `push_raw` make recorded bailout depths exactly match runtime; per-frame stack REGIONS from a monotonic cursor (nested natives start above ancestor slots — cross-call operand clobber structurally fixed), frame-relative bailout snapshots, overflow exit = BailOnEntry@pc0 (deep chains re-run interpreted from entry). **820 workspace passed, 0 failed.** CI: layout constants moved to rune_jit_baseline root (arch-gated re-exports fixed x86-64/clippy/MSRV/no-JIT jobs). |
| **v0.9.2** ✅ | **J1 JIT whitelist expansion**: `Div`/`Exp` compile natively via a shared `jit_binop_helper` (reuses the reserved JitHelpers slot — layout unchanged); `JumpIfNullOrUndefined` compiles exact nullish checks (optional chaining runs natively); `In`/`Instanceof` whitelisted with unconditional correct bailouts. **Trace fixes**: compiled-trace Add now does full §12.8.5 string-concat semantics (was silently NaN-ing `"x"+i` in hot loops — proven pre-existing); call-bearing loops never start recording (callee-tier-up depth-model abort, also pre-existing). 4 new tests — **815 workspace passed.** |
| **v0.9.1** ✅ | **C8 Object/Function/Math statics**: Math +18 methods (`round` with spec tie-toward-+∞ and the `0.4999…94` hazard guard, `trunc/sign/hypot/clz32/imul/cbrt/log/log2/log10/exp/sin/cos/tan/asin/acos/atan/atan2`) + 6 constants (LN2/LN10/LOG2E/LOG10E/SQRT1_2/SQRT2); Object `assign/is/getOwnPropertyNames/fromEntries/hasOwn/setPrototypeOf`; Function.prototype `apply` (spec-order IsCallable, array-like argArray). **Number-to-string fix**: floats now print `"Infinity"/"-Infinity"/"NaN"/"0"` per §7.1.12.1 (was Rust `"inf"`). 10 new integration tests — **811 workspace passed.** |
| **v0.9.0** ✅ | **C7 Array methods batch**: `Array.prototype.reverse/splice/concat/shift/unshift` completed + spec-audited (§23.1.3.x) — fixed the infinite-grow OOM in unshift/splice (discarded `grow()` pointers → `grow_dense_array` with forwarding resolution + root updates), `splice()` no-args no-op regression, ToIntegerOrInfinity coercions for start/deleteCount (`"1"`, `1.9`, `-1.5`, `NaN`), null/undefined receiver TypeErrors for all five, GC-move hardening (`resolve_forwarded`/`refresh_value`). **Pre-existing trace-recorder corruption gated**: recording a hot loop containing IC-bearing property loads corrupts execution (silent NaNs, proven on clean main via a dummy proto entry) — such loops now stay interpreted ("bail-to-interpreter stays correct"); numeric-only loops still trace. 15 new integration tests — **640/640 integration (3 ignored), 801 workspace.** |
| **v0.8.1** ✅ | **JIT GC duplicate-root crash + regression batch**: `SemiSpace::push_root` dedup + skip-gate `fi < source_frame_depth` for 14 pending ops, `new Object()` + `Object.prototype.*` + full Error ctors, `Array.prototype.join`, `new E().x` postfix, `RESERVED_SLOTS` 8, `extra_props` layout 40B. **620/620 integration (3 ignored), 781 workspace.** |
...
| **v1.0.0** 🚧 | **Conformance pass + JIT gap** — 12-slice plan (C0 done: baseline 620/620 + docs; C1-C8 RegExp anchors/word/backrefs/semantics, ToNumber(symbol), Error globals, Array/Object statics; J1-J3 JIT whitelist/float/x86-64+AFPC traces). Invariant: SIDT/AFPC/NaN-boxing/SIMD/GC preserved, bail-to-interpreter correct. |
| **v1.0.0-target** | Test262 80%+ (≥95% stretch), production hardening, fuzzing |
