...
- **Known gaps**: RegExp: no match/search/split, `replaceAll` function replacement not yet implemented, `this.prop++` not supported (Update only handles Identifier targets), `let` + `new` in function body has a scoping bug, compound `super.prop += val` not implemented.

### Next Steps — v0.5 (ordered by leverage)
1. `getter`/`setter` syntax — done
2. `class` private fields (`#`)
3. Compound assignment for `super.prop += val`
