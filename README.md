...
- **Array methods:** `filter`, `map`, `reduce`, `forEach`, `slice` (callback state machine, GC-safe across 200K elements), `find`, `findIndex`, `some`, `every`, `sort` (stable merge, user comparators, observable Get/Set element access, holes trail), `toSorted` (copy), `flat`, `flatMap`, `includes`, `push`, `pop`, `shift`, `unshift`, `splice`, `concat`, `reverse`, `indexOf`, `join`, `copyWithin`, `toSpliced`, `with`, `of`, `reduceRight`, `findLast`, `findLastIndex`, `lastIndexOf`
...
- **Array methods:** holes exist (elisions/`delete`/length-extension tails) but `map`/`filter` result shapes are unaudited with holes (length/positions); no `Array.from`; array `length` is a magic data property (no length descriptors, no `length`-`defineProperty`); `Object.freeze`/`seal` reject array receivers; `new Array(n)` fills `undefined` instead of holes; no 2^53 length guards (u32 lengths).
