...
- **JIT:** 62 opcodes whitelisted (out of ~93 opcode variants). Float Self-Tagging (NaN-boxing) eliminates all float heap allocation — all interpreter float paths use inline `Value::from_float64`. JIT has float64 Add promotion (full §12.8.5 string-concat semantics); Sub/Mul/Mod run natively as guarded Smi ops; **Div/Exp run natively** via `jit_binop_helper` (J1); float-binop helper extended for Sub/Mul/Mod/comparisons (J2 wiring pending — depth-model audit); optional chaining (`JumpIfNullOrUndefined`) runs native exact nullish branches; In/Instanceof bail to the interpreter. Compile-time `stack_depth` model + `push_raw` bail paths; every guard (shape-miss, Smi-check, overflow, call) bails to the interpreter with a validated pre-opcode snapshot. Phase F inlining shipped (5% on `jit_hot_function_1M`).
