...
| **5b** | Full function AOT compiler (bytecode→native for all opcodes) | 3d | 🔴 P0 | 🟡 In progress | AArch64 + x86-64 baseline JIT covers 46/61 opcodes (Smi arithmetic, comparison, bitwise, unary, branches, locals, property access, lexical scoping). Missing: floats, strings, calls, globals. |
...
| **5l** | Remaining JIT opcodes (floats, property access, calls) | 2d | 🟠 P1 | 🟡 In progress | Added lexical ops + LoadThis with VM callout framework. JIT now at 46/61. Next: bailout mechanism, float support. |
...
- JIT: 45/61 opcodes covered (Smi arithmetic, comparison, bitwise, unary, branches, locals, property access, lexical scoping).
...
**Gaps (documented):** No standard library, optimizing JIT (remaining 15/61 opcodes), modules, classes, async/await. 5–230× slower than V8 on hot loops. JIT covers 46/61 opcodes.
