...
- **Known gaps**: RegExp: no match/search/split, `replaceAll` function replacement not yet implemented, `this.prop++` not supported (Update only handles Identifier targets), `let` + `new` in function body has a scoping bug.

### Next Steps — v0.5 (ordered by leverage)
1. `getter`/`setter` syntax — done
2. Compound assignment for `super.prop += val` — done
3. `class` private fields (`#`)
