...
- **Array methods:** holes exist (elisions/`delete`/`length` tails/`length`-shrink, huge indices as named overflow props) but `map`/`filter` result shapes are unaudited with holes (length/positions); no `Array.from`; array `length` is a magic data property (no length descriptors, no `length`-`defineProperty`, direct assignment saturates instead of RangeError); `Object.freeze`/`seal` reject array receivers; `new Array(n)` fills `undefined` instead of holes; lengths clamp at 2^53-1 but dense storage caps at 2^32-1.
