...
- **Array methods:** `filter`, `map`, `reduce`, `forEach`, `slice` (callback state machine, GC-safe across 200K elements), `find`, `findIndex`, `some`, `every`, `sort` (stable merge, user comparators, observable Get/Set element access, holes trail), `toSorted` (copy), `flat`, `flatMap`, `includes`, `push`/`pop`/`shift`/`unshift`/`reverse` (spec-audited: generic receivers, holes, u64 lengths, 2^53 overflow throws, exact returns), `splice`, `concat`, `indexOf`, `join`, `copyWithin`, `toSpliced`, `with`, `of`, `reduceRight`, `findLast`, `findLastIndex`, `lastIndexOf`
