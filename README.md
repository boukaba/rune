...
- **Array methods:** `reduce`, `forEach`, `find`, `findIndex`, `some`, `every`, `sort` (stable merge, user comparators, observable Get/Set element access, holes trail), `toSorted` (copy), `flat`, `flatMap`, `includes`, `push`/`pop`/`shift`/`unshift`/`reverse`/`slice`/`splice`/`concat`/`map`/`filter` (spec-audited: generic receivers, holes, u64 lengths, 2^53 overflow throws, exact returns, IsConcatSpreadable, species-plain results, hole-preserving results), `indexOf`, `join`, `copyWithin`, `toSpliced`, `with`, `of`, `reduceRight`, `findLast`, `findLastIndex`, `lastIndexOf`
