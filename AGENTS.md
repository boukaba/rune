...
- **Class private fields (`#`) scaffold** — Lexer/parser/AST/bytecode/VM foundation. `TokenKind::Hash` in lexer. `Expr::PrivateMember` in AST. `LoadPrivateProperty`/`StorePrivateProperty`/`DefinePrivateField`/`PrivateNameScope` opcodes. VM throws clear "not yet implemented" TypeError. Private field declarations in class body not yet parsed. 3 new tests. 472/472 tests pass.
- **Known gaps**: RegExp: no match/search/split, `replaceAll` function replacement not yet implemented, `this.prop++` not supported (Update only handles Identifier targets), `let` + `new` in function body has a scoping bug. Private fields: only syntax scaffold — no runtime storage or class-body parsing yet.

### Next Steps — v0.5 (ordered by leverage)
1. `getter`/`setter` syntax — done
2. Compound assignment for `super.prop += val` — done
3. `class` private fields (`#`) — in progress (scaffold done, need runtime)
